...
class ViewGame(object):
    ...
    def draw_menu(self):
        ...
        self.rect1 = self.play_surface.get_rect(topleft=((self.width - fw) // 2, 330))
        self.background.blit(self.play_surface, ((self.width - fw) // 2, 330))

        computer_play_surface = font.render(computer_player_text, True, BLUE)
        fw, fh = font.size(computer_player_text)
        self.rect2 = computer_play_surface.get_rect(topleft=((self.width - fw) // 2, 380))
        self.background.blit(computer_play_surface, ((self.width - fw) // 2, 380))

        self.train_surface = font.render(train_text, True, RED)
        fw, fh = font.size(train_text)
        self.rect3 = self.train_surface.get_rect(topleft=((self.width - fw) // 2, 430))
        self.background.blit(self.train_surface, ((self.width - fw) // 2, 430))

        self.quit_surface = font.render(quit_text, True, BLUE)
        fw, fh = font.size(quit_text)
        self.rect4 = self.quit_surface.get_rect(topleft=((self.width - fw) // 2, 480))
